...
# 월별로 분리
d1 <- rent_cnt %>% filter(month==12) %>% select(c(대여소번호, 대여건수))

# 대여건수와 대여소정보 join
rent_map <- left_join(d1,station1,by='대여소번호')

# 대여소의 위도 경도 정보가 없는 데이터만 추출
out <- rent_map %>% filter(is.na(lon)) %>% select(c(대여소번호, 대여건수))
...
# 데이터 저장
write.csv(rent_info,"./pre_data/공공자전거_12월(대여소별대여건수,위도경도).csv")
# =========================================================================
# 지도 시각화
library(ggmap)
...
seoul_map <- get_map("seoul", zoom=11, maptype="terrain")
ggmap(seoul_map, extent = "device") + 
  geom_point(data=rent_map, aes(x=lon,y=lat, colour=대여건수),alpha=0.6 ) +
  scale_colour_gradient(low = "yellow", high = "green4", breaks=seq(0,5000,500) ) + 
  ggtitle("1월 대여소별 대여건수") + 
  theme(plot.title = element_text(hjust = 0.5, size = 24, face = "bold"))

# +
#   geom_text(data=rent_map_up_n, aes(x=lon, y=lat+0.01, 
#                             label=대여소명), size=1)
